...
This histogram shows the distribution of course evaluation scores (Evaluation Scores)
...
Distriution of course evaluation scores (Evaluation Scores) among professors who are native or non-native English speakers
...
Distribution of course evaluation scores (Evaluation Scores) among female and male professors
...
Relationship between beauty (Beauty) and the evaluation score of professors (Course Evaluation Scores)
...
The average number of bike rentals (Average Number of Rentals) during the different hours of the day (Hour of the Day) according to if it is a weekday or weekend.
...
ggplot(rents) + geom_col(aes(x = weathersit, y = avg_rents)) + facet_wrap(~workingday) + theme_classic() + labs(title = "Average Number of Bike Rentals in Different Weather Conditions According to If It Is a Workday", x = "Weather Conditions",
    y = "Average Number of Rentals")
```

The average number of bike rentals during the different weather conditions (Weather Condition)

According to the bar plots, first of all, weather has a pretty significant effect on bike rentals (Average Number of Rentals). When the weather is good (eg. clear, sunny, partly cloudy) (1), there are many people who rent bikes. However, when the weather is poor (eg.thunderstorms/snow/fog) (3), there is a lot less people who rented bikes. This relationship is demonstrated on both weekdays (1) and weekends (0). However, the average number of bike rentals also increased when it is a weekday and not a holiday, furthering showing that some people primarily rely on bikeshare services to commute to work.
...
Average boardings among September (Sep), October (Oct), and November (Nov) by day of the week in different hours of the day (Hour of the Day). 
...
The scatter plot shows the the number of boardings during the weekdays (weekday) or weekends (weekend) among the hour of the day at different temperatures.
...
Number of unique songs appearing at any position on the Billboard Top 100 chart in any week from 1959 to 2020. 
...
[To access my GitHub repository, click here](https://github.com/DanielWu3627/SDS315/tree/main)