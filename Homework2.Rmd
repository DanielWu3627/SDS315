...
## Part B
```{r}
ggplot(profs) + geom_boxplot(aes(x=eval)) + facet_wrap(~native) + theme_classic()
```
...
## Part C
```{r}
ggplot(profs) + geom_histogram(aes(x=eval)) + facet_wrap(~gender) + theme_classic()
```
...
## Part D
```{r}
ggplot(profs) + geom_jitter(aes(x=beauty, y = eval)) + theme_classic()
```
...
## Plot A
```{r}
ggplot(bikes) + geom_line(aes(x=hr, y = total)) + theme_classic()
```
...
## Plot B
```{r}
ggplot(bikes) + geom_line(aes(x=hr, y = total)) + facet_wrap(~workingday) + theme_classic()
```
...
ggplot(nine_am) + geom_col(aes(x = weathersit, y = total)) + facet_wrap(~workingday) + theme_classic()
```
...
## Part 1
```{r}
ggplot(capmetro_UT) + geom_line(aes(x = hour_of_day, y = boarding, color = month)) + facet_wrap(~day_of_week) + theme_classic()
```
...
## Part 2
```{r}
ggplot(capmetro_UT) + geom_jitter(aes(x = temperature, y = boarding, color = weekend)) + facet_wrap(~hour_of_day) + theme_classic()
```
