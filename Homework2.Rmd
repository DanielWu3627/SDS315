...
```
Average boarding among September, October, and November by day of the week. 
...
According to this line graph, the number of unique songs on the Billboard Top 100 peaked in the mid-1960s, and then rapidly declined unill around 2003. Then, it started to rapidly increase again, reachin a level near the mid-60s peak in 2020.

## Part C
```{r}
ten_week_hits <- filter(billb, weeks_on_chart >= 10)

artist_ten_week_hits <- aggregate(ten_week_hits$song ~ ten_week_hits$performer, FUN = function(x) length(unique(x)))

colnames(artist_ten_week_hits) <- c("performer", "ten_week_hits")

ten_week_hits <- filter(artist_ten_week_hits, ten_week_hits >= 30)

top_artists <- arrange(ten_week_hits, desc(ten_week_hits))
